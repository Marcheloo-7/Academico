...
Write-Host "  Este Core Asset incluye Dockerfiles, Docker Compose, CI/CD y ESLint."

function Install-CA011Dependencies {
  Write-Host "  Instalando dependencias de CA-011..." -ForegroundColor Cyan
  Push-Location $FRONTEND_DIR
  try {
...
if ($global:IncludeDevOpsTemplates) {
  ...
  Write-Host "  [X] CA-011 DevOps Templates omitido" -ForegroundColor DarkGray
}
...
def get_db_from_info(info: Info):
    ...
    if db is None:
        raise GraphQLError(
            `"No se pudo abrir la sesion de base de datos.`",
...
def get_usuario_actual(info: Info, *roles: str):
    ...
    if not auth_header or not auth_header.lower().startswith(`"bearer `"):
        raise GraphQLError(
            `"Debe iniciar sesion para realizar esta consulta.`",
    ...
    if not token:
        raise GraphQLError(
            `"El token de autenticacion esta vacio.`",
            extensions={`"code`": `"UNAUTHENTICATED`"},
        )
    ...
    try:
        ...
        raise GraphQLError(
            `"La sesion no es valida o ha expirado. Inicie sesion nuevamente.`",
            extensions={`"code`": `"UNAUTHENTICATED`"},
        ) from exc
...
Write-SkeletonFile -FilePath (Join-Path $FRONTEND_DIR "eslint.config.js") -Content $content_eslint_config
  ...
  Write-Host ""
  Write-Host "--- CA-011: DevOps Templates omitido ---" -ForegroundColor DarkGray
}
...
# Las consultas de estudiantes, docentes, cursos e inscripciones requieren JWT.
# En GraphiQL abra la pestana Headers y use:
...
class SafeGraphQLRouter(GraphQLRouter):
    MENSAJES_CONOCIDOS = {
        `"UNAUTHENTICATED`": `"Debe iniciar sesion o renovar una sesion expirada.`",
        `"FORBIDDEN`": `"No tiene permisos para realizar esta accion.`",
        `"BAD_USER_INPUT`": `"Revise los datos enviados e intente nuevamente.`",
    }
    ...
    async def process_result(self, request: Request, result: typing.Any) -> GraphQLHTTPResponse:
        ...
        if data.get(`"errors`"):
            for err in data[`"errors`"]:
                ...
                err[`"message`"] = self.MENSAJES_CONOCIDOS.get(
                    code,
                    `"Ocurrio un error inesperado al procesar la solicitud GraphQL.`",
                )
        return data
...
const errorLink = onError(({ graphQLErrors }) => {
  const sesionInvalida = graphQLErrors?.some(
...
if ($global:IncludeRolDocente) {
  ...
  if (Test-Path $requirementsPath) {
    ...
    Write-Host "  [INFO] requirements.txt no fue generado porque CA-011 esta omitido." -ForegroundColor Yellow
    Write-Host "         Se usaran las dependencias instaladas previamente por el script." -ForegroundColor Yellow
  }
  python seed_data.py
...
# ===========================================================================
# Revision final de funcionalidades opcionales
#
# Esta comprobacion se realiza justo antes de generar el esqueleto para que,
# si el usuario decide integrar las funcionalidades omitidas, todos los
# archivos compartidos (modelos, GraphQL, rutas y menu) se creen correctamente
# en una sola ejecucion.
# ===========================================================================

function Get-NotIntegratedFeatures {
  $features = [System.Collections.ArrayList]::new()

  if (-not $global:IncludeEstudiantes) {
    [void]$features.Add("Modulo Estudiantes")
  }
  if (-not $global:IncludeDocentes) {
    [void]$features.Add("Modulo Docentes")
  }
  if (-not $global:IncludeCursos) {
    [void]$features.Add("Modulo Cursos")
  }
  if (-not $global:IncludeInscripciones) {
    [void]$features.Add("Modulo Inscripciones")
  }
  if (-not $global:IncludeDevOpsTemplates) {
    [void]$features.Add("CA-011 DevOps Templates")
  }

  return $features
}

$notIntegratedFeatures = @(Get-NotIntegratedFeatures)

if ($notIntegratedFeatures.Count -gt 0) {
  Write-Host ""
  Write-Host "============================================================" -ForegroundColor Yellow
  Write-Host "  Funcionalidades que todavia no se integraran" -ForegroundColor Yellow
  Write-Host "============================================================" -ForegroundColor Yellow

  foreach ($feature in $notIntegratedFeatures) {
    Write-Host "  [X] $feature" -ForegroundColor DarkGray
  }

  Write-Host ""
  $installMissingFeatures = Ask-YesNo "  Desea instalar e integrar todas estas funcionalidades? (s/n)"

  if ($installMissingFeatures) {
    $installCA011Now = -not $global:IncludeDevOpsTemplates

    $global:IncludeEstudiantes = $true
    $global:IncludeDocentes = $true
    $global:IncludeCursos = $true
    $global:IncludeInscripciones = $true
    $global:IncludeDevOpsTemplates = $true
    ...
    Write-Host ""
    Write-Host "  [OK] Las funcionalidades pendientes seran integradas." -ForegroundColor Green
  }
  else {
    Write-Host ""
    Write-Host "  [INFO] Se continuara sin integrar las funcionalidades indicadas." -ForegroundColor Yellow
  }
}
else {
  Write-Host ""
  Write-Host "  [OK] Todas las funcionalidades opcionales fueron seleccionadas." -ForegroundColor Green
}

New-ProjectSkeleton

# Resumen definitivo de funcionalidades no integradas
$remainingFeatures = @(Get-NotIntegratedFeatures)

Write-Host ""
Write-Host "============================================================" -ForegroundColor Cyan
Write-Host "  Resumen final de funcionalidades" -ForegroundColor Cyan
Write-Host "============================================================" -ForegroundColor Cyan

if ($remainingFeatures.Count -eq 0) {
  Write-Host "  [OK] Todas las funcionalidades fueron integradas correctamente." -ForegroundColor Green
}
else {
  Write-Host "  Funcionalidades no integradas:" -ForegroundColor Yellow
  foreach ($feature in $remainingFeatures) {
    Write-Host "  [X] $feature" -ForegroundColor DarkGray
  }
}
